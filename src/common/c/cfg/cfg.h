...
static int cfg_get(cfg_t *cfg, void *var, const char *key, const char *fmt)
{
  int i;
...
static cfg_t *cfg_open(const char *fn)
{
  cfg_t *cfg;
  cfgln_t *cln;
  FILE *fp;
...
static void cfg_close(cfg_t *cfg)
{
  ssdelete(cfg->buf);
  free(cfg->lns);
  free(cfg->opts);
...
static int cfg_add(cfg_t *cfg, const char *key, const char *fmt,
...
static int cfg_match(cfg_t *cfg, unsigned flags)
{
...
static void cfg_dump(const cfg_t *cfg)
{
  int i, sz, len = 2;
  opt_t *ol = cfg->opts;
