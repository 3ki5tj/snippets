...
#define rng__open(seed) mtrng__open(seed)
#define rng__close(rng) mtrng__close(rng)
#define rng__save(rng, fn) mtrng__save(rng, fn)
#define rng__load(rng, fn) mtrng__load(rng, fn)
#define rng__randuint32(rng) mtrng__randuint32(rng)
#define rng__rand01(rng) mtrng__rand01(rng)
#define rng__randgaus(rng) mtrng__randgaus(rng)
...
__inline static void mtrng__init_from_seed(mtrng_t* mt, uint32_t seed)
{
  int k;
...
__inline static int mtrng__save(mtrng_t* mt, const char *fn)
{
  FILE *fp;
  int k;
...
/* load state from `fn' */
__inline static int mtrng__load(mtrng_t* mt, const char *fn)
{
  char s[64];
  int k, z, err = 1;
  FILE* fp;
...
/* load state from `fn' */
__inline static int mtrng__load_or_init_from_seed(mtrng_t* mt, const char *fn, unsigned long seed)
{
  int err = mtrng__load(mt, fn);

  if (err) {
    mtrng__init_from_seed(mt, seed);
  }
...
__inline static mtrng_t* mtrng__open(uint32_t seed)
{
  mtrng_t* mt;
...
__inline static void mtrng__close(mtrng_t* mt)
{
  free(mt);  
}
...
__inline static uint32_t mtrng__randuint32(mtrng_t* mt)
{
...
/* return an unsigned random number */
__inline static double mtrng__rand01(mtrng_t* mt)
{
  return mtrng__randuint32(mt) * (1.0/4294967296.0);
}
...
__inline static double mtrng__randgaus(mtrng_t* mt)
{
  double x, y, u, v, q;

  do {
    u = 1 - mtrng__rand01(mt);
    v = 1.7156*(mtrng__rand01(mt) - .5);  /* >= 2*sqrt(2/e) */
    x = u - 0.449871;
    y = fabs(v) + 0.386595;
