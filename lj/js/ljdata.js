...
LJData.prototype.getReportUP = function()
{
  ...
  s = 'Potential energy per particle: ' + roundto(aveU, 3);
  if ( this.Uref ) {
    s += ' (<span class="tag">REF:</span> ' + roundto(this.Uref, 3) + ")";
  }
  s += ".<br>";

  let aveP = this.aveP.getMean();
  s += 'Pressure: ' + roundto(aveP, 3);
  if ( this.Pref ) {
    s += ' (<span class="tag">REF:</span> ' + roundto(this.Pref, 3) + ")";
  }

  s += ".<br>" + this.count + " samples.";
...
LJData.prototype.getReport = function()
{
  ...
  if (this.isMC) {
    ...
    sinfo = "Acc. Ratio: " + roundto(100.0 * aveAcc, 2) + "%.<br>"
          + this.getReportUP();
  } else {
    ...
    sinfo = 'Temperature: '
            ...
            + ' (<span class="tag">REF:</span> ' + roundto(this.tp, 3) + ').<br>';
